<template>
    <div class="home-list-view">
        ...
        <div class="no-data" v-show="list.length==0"><i class="icon-z icon-z-nolist"></i><p>您还没有腰围记录</p></div>
        ...
        <mt-button type="primary" class="fixed-btn-b" @click="handleRecord">记腰围</mt-button>
    </div>
</template>
...
export default {
    ...
    created(){
      this.$store.dispatch('getRecodeList1', 4)
    },
    methods:{
        handleRecord(){
            this.$router.push({name:'recordWaistView'})
        },
        handleCurve(){
            this.$router.push({name:'waistCurveView'})
        },
        handleMore(){
            this.$router.push({name:'waistMoreView'})
        }
    },
