    ...
    <div class="my-report">
        <my-header :left="false" :title="title">
            ...
            <div slot="upload" class="filebox">
                <router-link :to="{name: 'uploadCompView'}">
                上传
                </router-link>
...
export default {
     ...
     computed:{
       imgList() {   //显示用的
       console.log(this.$store.getters.getContrastPhoto)
         return this.$store.getters.getContrastPhoto
       }
     },
     created(){
      this.$store.dispatch('getContrastPhoto')
     },
     methods:{
       goback(){
